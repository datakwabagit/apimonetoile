...
export class OfferingStockService {
  ...
  async incrementStock(offeringId: Types.ObjectId, name: string, quantity: number, icon?: string, category?: string): Promise<OfferingStock> {
    if (quantity <= 0) throw new BadRequestException('Quantité à ajouter invalide');
    return this.offeringStockModel.findOneAndUpdate(
      { offeringId },
      {
        $inc: { quantity },
        $setOnInsert: { name, icon, category },
      },
